...
describe('ActivityForm', () => {
    ...
    it('shows validation error when description is empty on submit', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByTestId('error-description')).toHaveTextContent('deskripsi aktivitas wajib diisi');
        });
    ...
    it('shows validation error when description exceeds 1024 characters', async () => {
        ...
        fireEvent.click(screen.getByTestId('submit-button'));
        await waitFor(() => {
            expect(screen.getByTestId('error-description')).toHaveTextContent('deskripsi terlalu panjang');
        });
        expect(mockInsertEvent).not.toHaveBeenCalled();
    });
    it('shows time validation error when end time <= start time', async () => {
        ...
        fireEvent.click(screen.getByTestId('submit-button'));
        await waitFor(() => {
            expect(screen.getByTestId('error-time')).toHaveTextContent('waktu selesai harus setelah waktu mulai');
        });
        expect(mockInsertEvent).not.toHaveBeenCalled();
    });
    it('disables submit button while submitting', async () => {
        ...
        let resolveInsert;
        mockInsertEvent.mockImplementation(() => new Promise((resolve) => { resolveInsert = resolve; }));
        render(_jsx(ActivityForm, {}));
        const textarea = screen.getByLabelText('Deskripsi');
        fireEvent.change(textarea, { target: { value: 'Test activity' } });
        fireEvent.click(screen.getByTestId('submit-button'));
...
//# sourceMappingURL=ActivityForm.test.js.map