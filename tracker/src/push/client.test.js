...
const { getSettings, postSettings, postSubscribe, deleteSubscribe, postDispatchTest } = await import('./client');
describe('Push_Service client', () => {
    ...
    describe('getSettings', () => {
        it('sends GET to /settings with Bearer token', async () => {
            ...
            expect(globalThis.fetch).toHaveBeenCalledWith(`${MOCK_PUSH_URL}/settings`, expect.objectContaining({
                method: 'GET',
        ...
        it('throws on non-ok response', async () => {
            vi.mocked(globalThis.fetch).mockResolvedValue(new Response('forbidden', { status: 403, statusText: 'Forbidden' }));
    ...
    describe('postSettings', () => {
        ...
        it('throws on non-ok response', async () => {
            vi.mocked(globalThis.fetch).mockResolvedValue(new Response('bad request', { status: 400, statusText: 'Bad Request' }));
            await expect(postSettings(MOCK_TOKEN, { time: 'invalid', timezone: '' })).rejects.toThrow('postSettings failed: 400 Bad Request');
        });
    });
    describe('postSubscribe', () => {
        it('sends POST to /subscribe with subscription JSON and Bearer token', async () => {
            ...
            expect(globalThis.fetch).toHaveBeenCalledWith(`${MOCK_PUSH_URL}/subscribe`, expect.objectContaining({
                method: 'POST',
                headers: expect.objectContaining({
                    Authorization: `Bearer ${MOCK_TOKEN}`,
                    'Content-Type': 'application/json',
        ...
        it('throws on non-ok response', async () => {
            vi.mocked(globalThis.fetch).mockResolvedValue(new Response('forbidden', { status: 403, statusText: 'Forbidden' }));
            await expect(postSubscribe(MOCK_TOKEN, { endpoint: '', keys: null })).rejects.toThrow('postSubscribe failed: 403 Forbidden');
        });
    });
    describe('deleteSubscribe', () => {
        ...
        it('throws on non-ok response', async () => {
            vi.mocked(globalThis.fetch).mockResolvedValue(new Response('not found', { status: 404, statusText: 'Not Found' }));
            await expect(deleteSubscribe(MOCK_TOKEN, 'https://example.com')).rejects.toThrow('deleteSubscribe failed: 404 Not Found');
        });
    });
    describe('postDispatchTest', () => {
        it('sends POST to /dispatch/test with Bearer token', async () => {
            ...
            expect(globalThis.fetch).toHaveBeenCalledWith(`${MOCK_PUSH_URL}/dispatch/test`, expect.objectContaining({
                method: 'POST',
                headers: expect.objectContaining({
                    Authorization: `Bearer ${MOCK_TOKEN}`,
                }),
        ...
        it('throws on non-ok response', async () => {
            vi.mocked(globalThis.fetch).mockResolvedValue(new Response('server error', { status: 500, statusText: 'Internal Server Error' }));
            await expect(postDispatchTest(MOCK_TOKEN)).rejects.toThrow('postDispatchTest failed: 500 Internal Server Error');
        });
    });
});
